...
# wy: DONE 2. 修改/继承这里来tokenize amr
def spider_amr_pre_process_function(
    batch: dict,
    ...
    inputs=[]
    for question, serialized_schema, amr in zip(batch["question"], batch["serialized_schema"], batch["amr"]):
        # serialized_schema_amr = amr + " $ " +  serialized_schema
        # eg 'How many heads of the departments are older than 56 ? | department_management | department : department_id , name , creation , ranking , budget_in_billions , num_employees | head : head_id , name , born_state , age | management : department_id , head_id , temporary_acting $ have-degree :ARG1 person :ARG0-of head :ARG1 department :quant amr-unknown :ARG2 old :ARG3 more :ARG4 age :ARG1 person :ARG2 temporal-quantity :quant 56 :unit year'
        serialized_schema_amr = serialized_schema + " $ " + amr
        inputs.append(spider_get_input(question=question, serialized_schema=serialized_schema_amr, prefix=prefix))
