...
from seq2seq.utils.spider import spider_add_serialized_schema, spider_pre_process_function, spider_amr_pre_process_function
...
def load_dataset(
    ...
    _spider_amr_pre_process_function = lambda batch, max_source_length, max_target_length: spider_amr_pre_process_function(
        batch=batch,
        max_source_length=max_source_length,
        max_target_length=max_target_length,
        data_training_args=data_training_args,
    ...
    _spider_amr_dataset_dict : Callable[[], DatasetDict] = lambda: datasets.load.load_dataset(
        path=data_args.dataset_paths['spider_amr'], cache_dir=model_args.cache_dir
    )
    ...
    elif data_args.dataset == "spider_amr":
        metric = _spider_metric()
        dataset_splits = prepare_splits(
            dataset_dict=_spider_amr_dataset_dict(),
            add_serialized_schema=_spider_add_serialized_schema,
            pre_process_function=_spider_amr_pre_process_function,
            **_prepare_splits_kwargs,
        )
    else:
